...
> **Source of Truth Document — v1.0.0**  
...
**Input/Output:**
- Input: `TaskNode` CRUD requests
- Output: Updated tree, events (`TaskAdded`, `TaskCompleted`, `TaskOverdue`)
...
**Handoff Notes:** Fully chat-buildable.
...
**Handoff Notes:** Fully chat-buildable. Toast UI rendering lives in TreeViewUI.
...
| Chat Complexity | **MEDIUM** (XAML structure chat-buildable; pixel-accurate verification requires runtime) |
...
public interface ITreeView
{
    void Show();
    void Hide();
    void ShowToast(ReminderEvent reminder);
}
```

**Performance Target:** Window show < 200ms warm  
**Handoff Notes:** XAML, ViewModels, bindings all chat-buildable. Visual polish, animations, and drag-drop edge cases finalized in Codex Phase 5E.
...
**Handoff Notes:** Fully chat-buildable. DPAPI calls verified in Codex.
...
**Handoff Notes:** Hash chain, redaction, and audit fully chat-buildable. `GetLastInputInfo` PInvoke finalized in Codex.
...
| Chat Complexity | **MEDIUM** (HTTP/signature logic chat-buildable; MSIX install handoff to Codex) |
...
**Handoff Notes:** Manifest fetch + verify chat-buildable; MSIX `Add-AppxPackage` invocation finalized in Codex.
...
| Chat Complexity | **MEDIUM** (redaction + queue chat-buildable; live SMTP/GitHub auth in Codex) |
...
**Handoff Notes:** Queue, redaction, payload schema chat-buildable. Credentials + live endpoints injected at runtime via Codex Phase 5E.
...
**TaskTree has NO external API integrations.** This section is preserved for future extensibility. Current selection logic short-circuits to "Local Only."
...
**Selection logic:** Always Local-only in v1.0.

---

## Section 7 — Core Engine (3-Tier Reminder Fallback Chain)

When a reminder fires, TaskTree tries three delivery tiers in order:
...
TaskTree v1.0 does not integrate with third-party apps. The default 4-tier cascade is documented here for future-proofing.
