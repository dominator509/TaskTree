...
- App startup resolves the existing AutoUpdater and BugReporter modules, injects TaskTreePaths-backed storage roots, installs the global crash hook, then starts the Orchestrator.
...
- Phase 5C offline validation is green with 396 non-live, non-performance contract tests plus 7 isolated measurable performance tests; the last fully converted SDK report measured 77.26% of production source (1,651/2,137 lines), and the current collector rerun produced a fresh ignored artifact. Phase 5D composition now resolves the persisted HotkeyManager, AutoUpdater, and BugReporter modules with TaskTreePaths-backed roots; the app installs the crash hook, Orchestrator verifies the audit chain before subscriptions and records failure without aborting startup, validates virtual-key bounds before native registration, preserves TaskNode metadata across storage snapshots, serializes settings, hotkey/session-lock/compliance idle-monitor lifecycles, unwinds failed orchestrator starts, completes best-effort shutdown, drains in-flight reminder delivery before stop, owns the scheduler timer reference across stop, marshals scheduler/session callbacks onto the WPF dispatcher, and enforces audit-chain sequence plus SecureStore key-path invariants, but real interactive E2E remains open. Phase 5E code paths now atomically promote staged packages and first-launch sentinels, reject unsafe manifest versions and incomplete offline package metadata, enforce channel/version/rollout eligibility again at apply time, verify offline package size before import, serialize updater checks/applies and state transitions, bound SMTP/GitHub delivery calls, require SMTP TLS, validate GitHub repository segments, keep crash-hook subscription idempotent, revalidate staged package integrity at apply time, promote file-drop reports through temporary files, serialize bug-report queue, limiter, and router delivery state, expose the PHI email allowlist as a snapshot, confine SecureStore key filenames, and retain delivered/failed queue records on the documented 7/30-day schedule; packaging preflight is hardened but stops on absent owner-supplied visual assets, while provider/package validation remains open. See the linked phase reports.
