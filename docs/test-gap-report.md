...
- `rtk C:\Users\domin\.dotnet\dotnet.exe test TaskTree.sln -c Release --no-build --filter "TestCategory!=Live&TestCategory!=Performance"`: passed, 14 assemblies, 360 tests, 0 failures, 0 skips.
...
- Built-in SDK coverage: `rtk C:\Users\domin\.dotnet\dotnet.exe test TaskTree.sln -c Release --no-build --filter "TestCategory!=Live&TestCategory!=Performance" --collect:"Code Coverage"`: passed, 360 tests, and produced a `.coverage` artifact under ignored `TestResults/`. SDK conversion to Cobertura reports 1,515/1,993 production source lines covered (76.02%), meeting the 75% target after hotkey integration.
