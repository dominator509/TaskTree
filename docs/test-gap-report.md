...
- Built-in SDK coverage: `rtk C:\Users\domin\.dotnet\dotnet.exe test TaskTree.sln -c Release --no-build --filter "TestCategory!=Live&TestCategory!=Performance" --collect:"Code Coverage"`: passed, 357 tests, and produced a `.coverage` artifact under ignored `TestResults/`. SDK conversion to Cobertura reports 1,505/1,980 production source lines covered (76.01%), meeting the 75% target after integrity hardening.
