...
- `rtk C:\Users\domin\.dotnet\dotnet.exe test TaskTree.sln -c Release --no-build --filter "TestCategory!=Live&TestCategory!=Performance"`: passed, 14 assemblies, 356 tests, 0 failures, 0 skips.
...
- Built-in SDK coverage: `rtk C:\Users\domin\.dotnet\dotnet.exe test TaskTree.sln -c Release --no-build --filter "TestCategory!=Live&TestCategory!=Performance" --collect:"Code Coverage"`: passed, 356 tests, and produced a `.coverage` artifact under ignored `TestResults/`. SDK conversion to Cobertura reports 1,465/1,923 production source lines covered (76.18%), meeting the 75% target.
