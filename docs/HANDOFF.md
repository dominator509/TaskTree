...
## Current Verified State (additive v1.0.83)
...
| 1.0.75 | 2026-08-16 | Compliance auto-logoff now hides an open main window through the existing event and dispatcher paths, with normal and failed-start subscription cleanup. See `docs/HANDOFF-v1.0.75-delta.md`. |
| 1.0.76 | 2026-08-16 | Runtime Light/Dark/System theme application and startup audit-chain warning plus atomic last-known-good export are implemented with focused regression coverage. See `docs/HANDOFF-v1.0.76-delta.md`. |
| 1.0.77 | 2026-08-16 | Hash-chain verification and persisted settings loading now fail closed on malformed values, with focused regression coverage and refreshed 410-test evidence. See `docs/HANDOFF-v1.0.77-delta.md`. |
| 1.0.78 | 2026-08-16 | Audit entries with empty actors now resolve the current Windows user SID at the central writer boundary; TaskEngine no longer emits usernames, with refreshed 411-test evidence. See `docs/HANDOFF-v1.0.78-delta.md`. |
| 1.0.79 | 2026-08-16 | SessionLock now rolls back an in-memory lock transition when its compliance audit fails, allowing a later observation to retry and preventing an unaudited state from suppressing future events. Focused regression coverage and refreshed 412-test evidence are recorded. See `docs/HANDOFF-v1.0.79-delta.md`. |
| 1.0.80 | 2026-08-16 | Hotkey configuration changes now roll back persisted and native state when compliance auditing fails, including removal of a newly created record, and suppress the change event. Focused regression coverage and refreshed 414-test evidence are recorded. See `docs/HANDOFF-v1.0.80-delta.md`. |
| 1.0.81 | 2026-08-16 | AutoUpdater now resets its shared state machine after `StateChanged` observer failures, including failures after the `Applied` transition, with focused regression coverage and refreshed 415-test evidence. See `docs/HANDOFF-v1.0.81-delta.md`. |
| 1.0.82 | 2026-08-16 | Settings save/reset transitions now roll back persisted state when compliance auditing fails, remove newly created records when needed, and suppress unaudited change events, with refreshed 418-test evidence. See `docs/HANDOFF-v1.0.82-delta.md`. |
| 1.0.83 | 2026-08-16 | Snooze create/clear/expiry transitions now roll back persisted state when compliance auditing fails, remove newly created records when needed, and suppress unaudited change events, with refreshed 422-test evidence. See `docs/HANDOFF-v1.0.83-delta.md`. |
