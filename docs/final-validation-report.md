...
- Offline contract suite: 14 assemblies, 362 passing tests, 0 failures, 0 skips. Performance tests run in a separate lane because their latency thresholds are timing-sensitive under parallel solution execution.
...
- Built-in SDK Code Coverage: 1,526/2,002 production source lines covered (76.22%) using `--collect:"Code Coverage"`; the ignored `TestResults/` artifact was converted to Cobertura for the production-source calculation after session-state serialization.
