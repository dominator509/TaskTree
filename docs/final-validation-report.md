...
- Offline contract suite: 14 assemblies, 360 passing tests, 0 failures, 0 skips. Performance tests run in a separate lane because their latency thresholds are timing-sensitive under parallel solution execution.
...
- Built-in SDK Code Coverage: 1,515/1,993 production source lines covered (76.02%) using `--collect:"Code Coverage"`; the ignored `TestResults/` artifact was converted to Cobertura for the production-source calculation after the hotkey integration.
