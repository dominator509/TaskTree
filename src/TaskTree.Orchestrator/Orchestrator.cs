...
namespace TaskTree.Orchestrator
{
    public sealed class Orchestrator : IOrchestrator, IDisposable
    {
        ...
        public async Task StartAsync(CancellationToken ct){ThrowIfDisposed();await _gate.WaitAsync(ct).ConfigureAwait(false);try{if(_running)throw new InvalidOperationException("Orchestrator already running.");_showTreeHandler=OnShowTreeRequested;_addTaskHandler=(s,e)=>_logger.LogInformation("AddTaskRequested");_exitHandler=(s,e)=>_logger.LogInformation("ExitRequested");_sessionLockHandler=OnSessionLockChanged;_trayHost.ShowTreeRequested+=_showTreeHandler;_trayHost.AddTaskRequested+=_addTaskHandler;_trayHost.ExitRequested+=_exitHandler;_sessionLock.SessionLockChanged+=_sessionLockHandler;try{_trayHost.Initialize();}catch(NotImplementedException ex){_logger.LogWarning($"TrayHost.Initialize stubbed: {ex.Message}");}await _sessionLock.StartAsync(ct).ConfigureAwait(false);await _reminderScheduler.StartAsync(ct).ConfigureAwait(false);await _reminderDeliveryService.StartAsync(ct).ConfigureAwait(false);await _compliance.AuditAsync(new AuditEntry{Module="Orchestrator",Action="Startup",Result="success",Timestamp=_clock.UtcNow}).ConfigureAwait(false);_running=true;}finally{_gate.Release();}}
        public async Task StopAsync(){ThrowIfDisposed();await _gate.WaitAsync().ConfigureAwait(false);try{if(!_running)return;if(_showTreeHandler is not null)_trayHost.ShowTreeRequested-=_showTreeHandler;if(_addTaskHandler is not null)_trayHost.AddTaskRequested-=_addTaskHandler;if(_exitHandler is not null)_trayHost.ExitRequested-=_exitHandler;if(_sessionLockHandler is not null)_sessionLock.SessionLockChanged-=_sessionLockHandler;try{_mainWindow?.Close();_mainWindow=null;_mainWindowViewModel=null;}catch(Exception ex){_logger.LogError($"MainWindow.Close failed: {ex.Message}");}await _sessionLock.StopAsync().ConfigureAwait(false);await _reminderDeliveryService.StopAsync().ConfigureAwait(false);await _reminderScheduler.StopAsync().ConfigureAwait(false);_trayHost.Dispose();await _compliance.AuditAsync(new AuditEntry{Module="Orchestrator",Action="Shutdown",Result="success",Timestamp=_clock.UtcNow}).ConfigureAwait(false);_running=false;}finally{_gate.Release();}}
        private async void OnShowTreeRequested(object? sender, EventArgs e){try{_mainWindowViewModel??=new MainWindowViewModel(_taskEngine,_clock,_logger,_settingsService);_mainWindow??=new MainWindow(_mainWindowViewModel);await _mainWindowViewModel.InitializeAsync().ConfigureAwait(true);if(!_mainWindow.IsVisible)_mainWindow.Show();else{_mainWindow.Visibility=System.Windows.Visibility.Visible;if(_mainWindow.WindowState==System.Windows.WindowState.Minimized)_mainWindow.WindowState=System.Windows.WindowState.Normal;_mainWindow.Activate();}_mainWindowHiddenBySessionLock=false;}catch(Exception ex){_logger.LogError($"OnShowTreeRequested failed: {ex.GetType().Name}: {ex.Message}");}}
        private void OnSessionLockChanged(object? sender, SessionLockChangedEventArgs e){try{if(e.IsLocked && _mainWindow is not null && _mainWindow.IsVisible){_mainWindow.Hide();_mainWindowHiddenBySessionLock=true;_logger.LogInformation("MainWindow hidden due to session lock.");}else if(!e.IsLocked && _mainWindowHiddenBySessionLock){_mainWindowHiddenBySessionLock=false;_logger.LogInformation("Session unlocked; MainWindow remains hidden until user reopens.");}}catch(Exception ex){_logger.LogError($"OnSessionLockChanged failed: {ex.GetType().Name}: {ex.Message}");}}
